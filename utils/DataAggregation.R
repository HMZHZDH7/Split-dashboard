...
ctrl_cols <- c("gender", "stroke_type", "prenotification", "imaging_done", "thrombolysis", 
               "hospital_stroke", "first_hospital")

Agg_cond_cols <- c("stroke_type", "thrombolysis", "thrombectomy", "hospital_stroke", 
                   "first_hospital", "post_acute_care", "discharge_destination")
...
hospital_Names <- c("Progress", "Paradise", "Angelvale", "Memorial", "Rose", "General", "Mercy", "Hope", 
                    "Samaritan")
...
keep_cols <- c(key_cols, "door_to_needle", "dnt_leq_60", "dnt_leq_45", "dnt_grt_45", "dgt_leq_120", 
               "dgt_leq_90", "dgt_grt_45", "rec_total_is", "isp_dis_antiplat", "mri_first_hosp", 
               "af_p_dis_anticoag", "sp_hosp_stroke_unit_ICU", "dysphagia_screening", "gender", "stroke_type",
               "prenotification", "imaging_done", "thrombolysis", "hospital_stroke", "first_hospital", 
               "dnt_missing")
...
charKPIcols <- c('thrombolysis','imaging_done','discharge_any_antiplatelet','discharge_any_anticoagulant',
                 'dysphagia_screening_type', 'hospitalized_in', 'prenotification')
doubleDutyCols <- c('imaging_done_dup','thrombolysis_dup','hospitalized_in_dup', 'prenotification_dup') 
...
df <- dataset[,] %>%
  ...
  mutate(imaging_done_dup = imaging_done,
         ...
         prenotification_dup=prenotification,
  ...
  mutate(QI = str_replace(QI, "imaging_done_dup", "imaging_done"),
         ...
         QI = str_replace(QI, "prenotification", "prenotification_dup"),
  ...
  mutate(#allConds=strsplit(incInAgg, "&"),
    ...
    qual4agg = ifelse(QI!="prenotification",qual4agg, TRUE),
